...
const TRANSLATIONS = {
  'en': {
    ...
    'gs.step4_desc':  'The model setup wizard detects every NVIDIA and AMD GPU on the system (multi-GPU is fully supported — 2× NVIDIA, 1× NVIDIA + 1× AMD, etc.), lets you pick one, and installs / configures the backend. The chosen vendor is saved to <code style="font-family:var(--font-mono)">config.model.gpu_vendor</code> and enforced at runtime — picking AMD on a machine that also has NVIDIA actually routes inference to AMD. It also installs Ollama on demand when you pick that backend — so there is no separate Ollama step.',
  ...
  'pt': {
    ...
    'gs.step4_desc':  'O wizard de setup detecta cada GPU NVIDIA e AMD do sistema (multi-GPU totalmente suportado — 2× NVIDIA, 1× NVIDIA + 1× AMD, etc.), deixa você escolher uma e instala/configura o backend. O vendor escolhido é salvo em <code style="font-family:var(--font-mono)">config.model.gpu_vendor</code> e honrado em runtime — escolher AMD numa máquina que também tem NVIDIA realmente roteia a inferência para a AMD. Também instala o Ollama sob demanda quando você escolhe esse backend — sem etapa Ollama separada.',
